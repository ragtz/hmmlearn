...
from .base import _BaseHMM, _BaseHMMwT
from .utils import iter_from_X_lengths, normalize, fill_covars

__all__ = ["GMMHMM", "GaussianHMM", "GaussianHMMwT", "MultinomialHMM"]
...
class GaussianHMMwT(_BaseHMMwT):
    """Hidden Markov Model with Gaussian emissions and terminal states.
    ...
    params : string, optional
        Controls which parameters are updated in the training
        process.  Can contain any combination of 's' for startprob,
        't' for transmat, 'e' for endprob, 'm' for means and 'c' for 
    ...
    Examples
    --------
    >>> from hmmlearn.hmm import GaussianHMMwT
    >>> GaussianHMMwT(n_components=2)
    ...                             #doctest: +ELLIPSIS +NORMALIZE_WHITESPACE
    GaussianHMMwT(algorithm='viterbi',...
    """
    def __init__(self, n_components=1, covariance_type='diag',
                 min_covar=1e-3, startprob_prior=1.0,
                 transmat_prior=1.0, endprob_prior=1.0,
                 means_prior=0, means_weight=0,
                 covars_prior=1e-2, covars_weight=1,
                 algorithm="viterbi", random_state=None,
                 n_iter=10, tol=1e-2, verbose=False,
                 params="stemc", init_params="stemc"):
        _BaseHMMwT.__init__(self, n_components, startprob_prior=startprob_prior,
                            transmat_prior=transmat_prior, endprob_prior=endprob_prior,
    ...
    def _check(self):
        super(GaussianHMMwT, self)._check()
    ...
    def _init(self, X, lengths=None):
        super(GaussianHMMwT, self)._init(X, lengths=lengths)
    ...
    def _initialize_sufficient_statistics(self):
        stats = super(GaussianHMMwT, self)._initialize_sufficient_statistics()
        stats['post'] = np.zeros(self.n_components)
        stats['obs'] = np.zeros((self.n_components, self.n_features))
        stats['obs**2'] = np.zeros((self.n_components, self.n_features))
        if self.covariance_type in ('tied', 'full'):
    ...
    def _accumulate_sufficient_statistics(self, stats, obs, framelogprob,
                                          posteriors, fwdlattice, bwdlattice):
        super(GaussianHMMwT, self)._accumulate_sufficient_statistics(
            stats, obs, framelogprob, posteriors, fwdlattice, bwdlattice)
    ...
    def _do_mstep(self, stats):
        super(GaussianHMMwT, self)._do_mstep(stats)
