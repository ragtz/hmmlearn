...
class _BaseHMMwT(BaseEstimator):
    """Base class for Hidden Markov Models with terminal states.
    ...
    endprob_prior : array, shape (n_components, ), optional
        Parameters of the Dirichlet prior distribution for
        :attr:`endprob_`.
    ...
    endprob\_ : array, shape (n_components, )
    ...
    def __init__(self, n_components=1, startprob_prior=1.0, 
        ...
        self.startprob_prior = startprob_prior
        self.transmat_prior = transmat_prior
        self.endprob_prior = endprob_prior
        self.algorithm = algorithm
        self.random_state = random_state
        self.n_iter = n_iter
        self.tol = tol
    ...
    # TODO: update to include endprob
    def _do_viterbi_pass(self, framelogprob):
        n_samples, n_components = framelogprob.shape
        state_sequence, logprob = _hmmc._viterbi_wt(
            n_samples, n_components, log_mask_zero(self.startprob_),
            log_mask_zero(self.transmat_), log_mask_zero(self.endprob_),
            framelogprob)
        return logprob, state_sequence
    ...
    def _do_forward_pass(self, framelogprob):
        n_samples, n_components = framelogprob.shape
        fwdlattice = np.zeros((n_samples, n_components))
        _hmmc._forward_wt(n_samples, n_components,
                          log_mask_zero(self.startprob_),
                          log_mask_zero(self.transmat_),
                          log_mask_zero(self.endprob_),
                          framelogprob, fwdlattice)
        return logsumexp(fwdlattice[-1]), fwdlattice
    ...
    def _do_backward_pass(self, framelogprob):
        n_samples, n_components = framelogprob.shape
        bwdlattice = np.zeros((n_samples, n_components))
        _hmmc._backward_wt(n_samples, n_components,
                           log_mask_zero(self.startprob_),
                           log_mask_zero(self.transmat_),
                           log_mask_zero(self.endprob_),
                           framelogprob, bwdlattice)
    ...
    def _check(self):
        ...
        self.endprob_ = np.asarray(self.endprob_)
        if len(self.endprob_) != self.n_components:
            raise ValueError("endprob_ must have length n_components")
        if not np.allclose(self.endprob_.sum(), 1.0):
            raise ValueError("endprob_ must sum to 1.0 (got {0:.4f})"
                             .format(self.endprob_.sum()))
    ...
    def _initialize_sufficient_statistics(self):
        ...
        end : array, shape (n_components, )
            An array where the i-th element corresponds to the posterior
            probability of the last sample being generated by the i-th
            state.
        """
        stats = {'nobs': 0,
                 'start': np.zeros(self.n_components),
                 'trans': np.zeros((self.n_components, self.n_components)),
                 'end': np.zeros(self.n_components)}
        return stats
    ...
    def _do_mstep(self, stats):
        ...
        if 't' in self.params:
            ...
            normalize(self.transmat_, axis=1)
        if 'e' in self.params:
            endprob_ = self.endprob_prior - 1.0 + stats['end']
            self.endprob_ = np.where(self.endprob_ == 0.0,
                                     self.endprob_, endprob_)
            normalize(self.endprob_)
